<template>
  <div class="push-wrap">
    <div ref="topRef" class="left">
      <div ref="containerRef" class="container">
        ...
        <div class="record-ico" @click="handleRecordVideo">
          <n-popover placement="top" trigger="hover" :flip="false">
            <template #trigger>
              <n-icon size="26" :color="recording ? 'red' : '#3f7ee8'">
        ...
        <canvas id="pushCanvasRef" ref="pushCanvasRef"></canvas>

        <div v-if="appStore.allTrack.filter((item) => !item.hidden).length <= 0" class="add-wrap">
          <n-space>
            <n-button v-for="(item, index) in allMediaTypeList" :key="index" class="item"
      ...
      <div ref="bottomRef" class="room-control">
        <div class="info">
          <div class="avatar" :style="{ backgroundImage: `url(${userStore.userInfo?.avatar})` }"></div>
          <div class="detail">
            <div class="top">
              <n-input-group>
                <n-input v-model:value="roomName" size="small" placeholder="输入房间名" :style="{ width: '50%' }" />
                <n-button size="small" type="primary" @click="confirmRoomName">
                  确定
                </n-button>
              </n-input-group>
            </div>
        ...
        <div class="rtc">
          <div class="item">
            ...
            <div class="down">
              <n-select v-model:value="currentMaxBitrate" :options="maxBitrate" />
            </div>
          </div>
          <div class="item">
            <div class="txt">帧率设置</div>
            <div class="down">
              <n-select v-model:value="currentMaxFramerate" :options="maxFramerate" />
            </div>
          </div>
          <div class="item">
            <div class="txt">分辨率设置</div>
            <div class="down">
              <n-select v-model:value="currentResolutionRatio" :options="resolutionRatio" />
            </div>
          </div>
        </div>
        <div class="other">
          <div class="top">
            ...
            <div class="item" v-if="NODE_ENV === 'development'">
              {{ liveRoomTypeEnumMap[appStore.liveRoomInfo?.type || ''] }}
            </div>
          </div>
          <div class="bottom">
            <n-button v-if="!roomLiving" type="info" size="small" @click="handleStartLive">
              开始直播
            </n-button>
            <n-button v-else type="error" size="small" @click="handleEndLive">
              结束直播
            </n-button>
          </div>
        </div>
    ...
    <div class="right">
      <div class="resource-card">
        ...
        <div class="list">
          <div v-for="(item, index) in appStore.allTrack.filter(
              (item) => !item.hidden
            )" :key="index" class="item" @click="handleActiveObject(item)">
            <div class="item-left">
              <div class="control-item" @click="handleEye(item)">
                <n-icon size="16" v-if="item.openEye && item.type !== MediaTypeEnum.microphone">
            ...
            <div class="control">
              <div v-if="item.audio === 1" class="control-item" @click.stop="handleChangeMuted(item)">
                <n-popover placement="top" trigger="hover" :flip="false">
                  <template #trigger>
                    <n-icon size="16">
                  ...
                  <div class="slider">
                    <n-slider :value="item.volume" :step="1" @update-value="(v) => handleChangeVolume(item, v)" />
                  </div>
                </n-popover>
              </div>
              <div class="control-item" @click="handleEdit(item)">
                <n-icon size="16">
                  <CreateOutline></CreateOutline>
                </n-icon>
              </div>
              <div class="control-item" @click.stop="handleDel(item)">
                <n-icon size="16">
                  <Close></Close>
                </n-icon>
              </div>
        ...
        <div class="bottom">
          <n-button size="small" type="primary" @click="showSelectMediaModalCpt = true">
            添加素材
          </n-button>
        </div>
      </div>
      <div class="danmu-card">
        ...
        <div class="list-wrap">
          <div ref="danmuListRef" class="list">
            <div v-for="(item, index) in damuList" :key="index" class="item">
              <template v-if="item.msgType === DanmuMsgTypeEnum.danmu">
                ...
                <span>：</span>
                <span class="msg" v-if="item.msgIsFile === WsMessageMsgIsFileEnum.no">
                  {{ item.msg }}
                </span>
                <div class="msg img" v-else>
                  <img :src="item.msg" alt="" @load="handleScrollTop" />
                </div>
              </template>
        ...
        <div class="send-msg" v-loading="msgLoading">
          <div class="control">
            <div class="emoji-list" v-if="showEmoji">
              <div class="item" v-for="(item, index) in emojiArray" :key="index" @click="handlePushStr(item)">
                {{ item }}
              </div>
            </div>
            <div class="ico face" title="表情" @click="showEmoji = !showEmoji"></div>
  ...
  <div>
    <SelectMediaModalCpt v-if="showSelectMediaModalCpt" :all-media-type-list="allMediaTypeList"
      @close="showSelectMediaModalCpt = false" @ok="handleShowMediaModalCpt"></SelectMediaModalCpt>

    <MediaModalCpt v-if="showMediaModalCpt" :is-edit="isEdit" :media-type="currentMediaType"
      :init-data="currentMediaData" @close="showMediaModalCpt = false" @add-ok="addMediaOk" @edit-ok="editMediaOk">
    </MediaModalCpt>

    <OpenMicophoneTipCpt v-if="showOpenMicophoneTipCpt" @close="showOpenMicophoneTipCpt = false"></OpenMicophoneTipCpt>

    <NoLiveTipModalCpt v-if="showNoLiveTipModalCpt" @close="showNoLiveTipModalCpt = false"></NoLiveTipModalCpt>
  </div>
</template>
